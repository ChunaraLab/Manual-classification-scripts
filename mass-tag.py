...
regex = sys.argv[1]
newtag = sys.argv[2]
matches = db.hpv.find({'lang':'en', tagField: { '$exists': True, '$nin': [1, 2, 3, 4]}, tweetText: { '$regex': regex } } )
...
for tweet in matches:
	db.hpv.update({ tweetID:tweet[tweetID]}, {'$set':{ tagField:newtag }}, upsert=False, multi=True)
print 'Done! Running query again to check:'
matches = db.hpv.find({'lang':'en', tagField: { '$exists': True, '$nin': [1, 2, 3, 4]}, 't': { '$regex': regex } } )
